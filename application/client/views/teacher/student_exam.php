...
<div class="anserBox">
    <div class="anserTitle">
        <p class="title"><span><?php echo $data[0]['TaskName'];?></span>[<?php echo '考生 : '.$data[0]['UserName'].' 得分: '.$data[0]['TaskScore'];?>]<span></span></p>
            ...
            foreach($data as $k=>$v){
                ...
                <div class="question">
                    <?php if($v['QuestionType'] == 1){ $letter=65; ?>
                        <div class="queTitle <?php if($v['QuestionAnswer'] == $v['Answer']){ echo 'queTitleRight';}else{ echo 'queTitleError';}?>">
                        ...
                        <div class="anser">
                            <ul>
                                <?php foreach($QuestionChoose as $key=>$value){ ?>
                                    <li><input class="submitanswers" status="1" type="radio" questionid="<?php echo $v['QuestionID']; ?>" name="<?php echo $v['QuestionID']; ?>" value="<?php echo $value; ?>" <?php echo $v['Answer'] == $value ? 'checked' : '' ?> ><?php echo chr($letter++); ?>.<?php echo $value; ?></li>
                                <?php } ?>
                            </ul>
                        </div>
                    <?php } ?>
                    <?php if($v['QuestionType'] == 2){ $letter=65;
                        ...
                        ?>
                        <div class="queTitle <?php if(count($arr_diff) > 0){echo 'queTitleError';}else{ echo 'queTitleRight';}?>">
                            <div class="queNumber"><span><?php echo $k+1;?>. (本题 <?php echo $v['QuestionScore'];?> 分)</span>
                                <?php echo $scene_str;?>
                            </div>
                            <div class="queTxt markdown-body" id="qus_<?php echo $k+1;?>" dataurl="<?php echo $dataUrl;?>" dataname="<?php echo $dataName;?>"></div>
                        ...
                        <div class="anser">
                            <ul>
                                <?php foreach($QuestionChoose as $key=>$value): ?>
                                    <li><input class="submitanswers" status="2" type="checkbox" questionid="<?php echo $v['QuestionID']; ?>" name="<?php echo $v['QuestionID']; ?>[]" value="<?php echo $value; ?>" <?php echo @in_array($value,$okanswer) ? 'checked' : ''; ?>><?php echo chr($letter++); ?>.<?php echo $value; ?></li>
                                <?php endforeach; ?>
                            </ul>
                        </div>
                    <?php } ?>
                    <?php if($v['QuestionType'] == 3){ ?>
                        ...
                        <div class="anser">
                            <ul>
                                <input class="submitanswers" status="3" type="radio" questionid="<?php echo $v['QuestionID']; ?>" name="<?php echo $v['QuestionID']; ?>" value="对" <?php if($v['Answer'] == '对'){ echo 'checked';}?>>正确
                                <input class="submitanswers" status="3" type="radio" questionid="<?php echo $v['QuestionID']; ?>" name="<?php echo $v['QuestionID']; ?>" value="错" <?php if($v['Answer'] == '错'){ echo 'checked';}?>>错误
                            </ul>
                        </div>
                    <?php } ?>
                    <?php if($v['QuestionType'] == 4){ ?>
                        ...
                        <div class="anser">
                            <input class="submitanswers" status="4" type="text"  questionid="<?php echo $v['QuestionID']; ?>" name="<?php echo $v['QuestionID']; ?>" value="<?php echo $v['Answer'] ?>" placeholder="请输入答案...">
                        </div>
                    <?php } ?>
