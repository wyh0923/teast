...
<head>
    ...
    <script src="<?php echo base_url() ?>resources/js/teacher/exam_task.js"></script>
...
<div class="frame">
    <div class="main clearfix">
        ...
        <div class="content">
            ...
            <?php $diff = array('初级','中级','高级');
            if($total > 0){ ?>
            <table class="testPaperList">
                <thead>
                <tr class="table-title">
                    <td width="60"><input type="checkbox" id="checkAll">全选</td>
                    <td width="300">考试名称</td>
                    <td width="80">任务类型</td>
                    <td width="60">难度</td>
                    <td id="" width="100"><a href="<?php echo $Sort["CreateTime"]["url"]; ?>">下发日期<i class="<?php echo $Sort["CreateTime"]["icon"]; ?>"></i></a></td>
                ...
                <tr>
                    <td><?php if($val['TaskTypeJudge'] == 2){?>
                            <input type="checkbox" data-code="<?php echo $val['TaskCode'];?>" onclick="checkThis('#exam_task','checkAll')" name="checkTask">
                        <?php } ?></td>
                    <td title="<?php echo $val['TaskName'];?>" class="">
                        <a class="forRed" href="<?php echo site_url().'Education/examquestion?examid='.$val['ExamID'];?>" target="_blank"><?php echo $val['TaskName'];?></a>
                    </td>
                    <td><?php if($val['TaskTargetType'] == 1){
                            echo '学员任务';
                    ...
                    <td >
                        <a title="<?php echo $val['Progress'];?>%" href="<?php echo site_url().'Education/examdetail?taskcode='.$val['TaskCode'];?>">
                    ...
                    <td>
                        <?php
                        $end = '<a class="endExam" onclick="endExam(this)" data-code="'.$val["TaskCode"].'" scenetaskid="'.$val["SceneTaskID"].'" href="javascript:;" ><i class="fa fa-stop-circle-o"></i>结束</a>';
                        if($val['TaskTypeJudge'] == 2){
                            $end = '<a class="endspan">已结束</a>';
                        }
                        echo $end;
                        ?>
                        <a href="javascript:;" class="forRed delExam" onclick="delExam(this)" data-code="<?php echo $val['TaskCode'];?>" scenetaskid="<?php echo $val['SceneTaskID'];?>"><i class="fa fa-trash "></i>删除</a>
                    </td>
                </tr>
                <?php } ?>
                </tbody>
...
<div class="popUpset animated " id="examBox">
    <form action="" method="post">
        ...
        <div class="infoBox">
            <p class="promptNews">请确认是否删除所有选中的考试任务?</p>
            <div class="btnBox">
                <a href="javascript:;" class="publicOk " id="delExamBtn">确定</a>
...
<div class="popUpset animated " id="delBox">
    <form action="" method="post">
        <div class="popTitle">
            ...
            <input type="hidden" id="delSceneTaskID" value=""/>
        </div>
        <div class="infoBox">
...
<div class="popUpset animated " id="endBox">
    <form action="" method="post">
        <div class="popTitle">
            ...
            <input type="hidden" id="endSceneTaskID" value=""/>
        </div>
        <div class="infoBox">
