...
$.fn.Huploadify = function(opts){
	var itemTemp = '<div id="${fileID}" class="uploadify-queue-item"><div class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><span class="up_filename">${fileName}</span><span class="uploadbtn">上传</span><span class="delfilebtn">取消</span></div>';
	var defaults = {
	...
	this.each(function(){
		var _this = $(this);
		//先添加上file按钮和上传列表
		var instanceNumber = $('.huploadifyBox').length+1;
		var inputStr = '<div class="huploadifyBox"><input id="select_btn_'+instanceNumber+'" class="selectbtn" style="display:none;" type="file" name="fileselect[]"';
		inputStr += option.multi ? ' multiple' : '';
		inputStr += ' accept="';
		inputStr += getAcceptString(option.fileTypeExts);
		inputStr += '"/>';
		//<<<<<<[edited by b1953a begin][for style]
		inputStr += '<input type="text" class="file_info_show_box" id="file_info_show_box_' + instanceNumber + '" disabled="true" value="' + option.fileInfoText + '">';
		inputStr += '<a class="uploadify-button" id="file_upload_'+instanceNumber+'-button" href="javascript:;"><i class="fa fa-upload uploadify-button_icon"></i></a>';
		
		//inputStr += '<a id="file_upload_'+instanceNumber+'-button" href="javascript:void(0)" class="uploadify-button">';
		//inputStr += option.buttonText;
		//inputStr += '</a>';

		//<<<<<<[edited by b1953a end]
		var uploadFileListStr = '<div id="file_upload_'+instanceNumber+'-queue" class="uploadify-queue"></div></div>';
		_this.append(inputStr+uploadFileListStr);	
		
		
		//创建文件对象
	  fileObj = {
	  		insNum:instanceNumber,
	  	uploadAllowed: true,
		  ...
		  funSelect: function(files){
				for(var i=0,len=files.length;i<len;i++){
					...
					if(option.auto){
						...
						$('#file_info_show_box_' + instanceNumber).hide();
						$('#file_upload_' + instanceNumber + '-button').hide();
					}
					...
					if(option.breakPoints){
						...
						var uploadedSize = 0;  //by B1953a: 强制关闭断点续传，但不关闭分片上传
						//console.log("uploadedSize in funSelect: " + uploadedSize);
				  	//先设置进度条为原来已上传的文件大小
		  ...
		  onProgress: function(file, loaded, total) {
                ...
                var percentText = Math.min(99.99,percent)+'%';//校正四舍五入的计算误差

			  //console.log(percentText);
				if(option.showUploadedSize){
				    //console.log(formatFileSize(total*percent/100));
					//eleProgress.nextAll('.progressnum .uploadedsize').text(formatFileSize(loaded));
					eleProgress.nextAll('.progressnum').find('.uploadedsize').text(formatFileSize(total*percent/100));
		  ...
		  funDeleteFile: function(index) {
			  for (var i = 0,len=this.fileFilter.length; i<len; i++) {
					  ...
					  if (file.index == index) {
						  ...
						  _this.find('#fileupload_'+instanceNumber+'_'+index).hide();
						  //_this.find('#file_info_show_box_' + instanceNumber).val("");
						  _this.find('#file_info_show_box_' + instanceNumber).fadeIn();
						  _this.find('#file_upload_' + instanceNumber + '-button').fadeIn();
					  ...
					  if (xhr.readyState == 4) {
						  ...
						  if (xhr.status == 200) {
								...
								if(returnData.success){
									...
									$('#file_info_show_box_' + instanceNumber).val(returnData.showinfo);
								}
								...
								if(fileObj.uploadOver){
								  ...
								  //在指定的间隔时间后删掉进度条
								  setTimeout(function(){
									_this.find('#fileupload_'+instanceNumber+'_'+originalFile.index).hide();
									$('#file_info_show_box_' + instanceNumber).show();
									$('#file_upload_' + instanceNumber + '-button').show();
	...
	return {
		clearSelf : function(){
			$('#file_info_show_box_' + fileObj.insNum).val("");
		},
		...
		setFileInfo : function(info){
			alert(info);
			//$('#file_info_show_box_' + instanceNumber).val(info);
		},
		stop : function(){
  		fileObj.uploadOver = false;
