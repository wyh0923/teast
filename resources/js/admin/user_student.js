...
$(function(){
    $(".fa-search").click(function(){
        ...
        window.location.href= site_url + '/User/student' +str+ "/search/"+encodeURI(translate(search));
    });
        ...
        $.ajax({
            ...
            success:function(message){
                if(message.code=='0000'){
                    ...
                    console.log(message.data['ClassName']);
        ...
        $.ajax({
            ...
            success:function(message){
                ...
                if(message.code=='0000'){
                    ...
                    setTimeout(function () {
                        window.location = site_url+"/User/student";
                    },2000);
                }else{
    ...
    $('#CreateTime,#UserSex,#UserDepartment').click(function(){
        ...
        if(code == 'DESC'){
            location.href = site_url+'/User/student' + str + '/sort/'+field+' ASC';
        }else if(code == 'ASC'){
            location.href = site_url+'/User/student' + str + '/sort/'+field+' DESC';
        }else{
            location.href = site_url+'/User/student' + str + '/sort/'+field+' DESC';
        }
    });
...
function searchForTime(){
    if($("#stime").val() != "" && $("#etime").val() != ""){
        if($("#stime").val() >= $("#etime").val()){
            ...
            var str = '';
            if (search != '')str += '/search/'+encodeURI(translate(search));
            window.location.href =  site_url+'/User/student' + str +'/time/'+ $("#stime").val() + "_" + $("#etime").val()
        }
    }
...
function clearTime() {
    if ($("#stime").val() == "" && $("#etime").val() == "") {
        var str = '';
        if (search != '')str += '/search/' + encodeURI(translate(search));
        window.location.href = site_url + '/User/student' + str;
    }
}
