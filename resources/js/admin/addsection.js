...
$(function(){
   //小节新增图片
    ...
    var upadd = $('#videoUploadBox').Huploadify({
        ...
        uploader: site_url + '/Adminsubject/uploadvideo',//服务器端脚本文件路径
    ...
    var uploadTool = $('#addDatas').Huploadify({
        ...
        fileSplitSize:2048*2048,
        buttonText:'上传资料',
        uploader:site_url+'/Adminsubject/uploadtoolRes',
    ...
    var upquestion = $('#uploadQuestion').Huploadify({
        ...
        fileSplitSize: 2048 * 2048,
        buttonText: '上传资源',
        uploader: site_url + '/Adminsubject/accessory',//服务器端脚本文件路径
        onUploadComplete: function (messfileObj, info, responseage) {
    ...
    $(".selectCtfButton").click(function(){
      $(".goSearch input").val('');//初始化弹窗搜索
        sapGetData(site_url+'/Adminsubject/ctflist',sapSucCtf,"ctfPage");
   ...
   $(".selectSceneButton").click(function(){
        $(".goSearch input").val('');//初始化弹窗搜索
        sapGetData(site_url+'/Adminsubject/scenelist',sapSucScene,"secePage");
   ...
   //点击新增图片
    ...
    $("#addDataOK").click(function(){
        ...
        $.ajax({
            url: site_url + "/Adminsubject/addmaterial",
            type: 'post',
            async: false,
            data: {
    ...
    //点击新增题目弹窗里面的保存按钮
     $("#saveQueList").click(function(){
        ...
        $('#addquesTable tr').each(function () {
         ...
         $.ajax({
               url: site_url + "/Adminsubject/doaddquestion",
               type: 'post',
               data: {
                   'QuestionDesc': tigan,
                   'QuestionType': type,
    ...
    $("#selectDataBtn").click(function(){
        tongjidata();
        $(".goSearch input").val('');//初始化弹窗搜索
        sapGetData(site_url+'/Adminsubject/datumlist', sapSucData, "dataPage");
    ...
    $("#selectQuesBtn").click(function(){
      ...
      $("#sapSearch_choseQuesPage").val('')
    	sapGetData(site_url+'/Adminsubject/all_question', sapSucQuesList, "choseQuesPage");
    ...
    $("#saveAllNews").click(function(){
       ...
       $('#allEroar').html("");
        ...
        {
            $.ajax({
                url:site_url+"/Adminsubject/dosection",
                type:'post',
                data:{
                    'cid':PackageCode,
                ...
                success:function(msg){
                    ...
                    if(msg.code != 0){
                        ...
                        $('#allEroar').text('新增成功');
                        setTimeout(function(){
                            window.location.href =site_url+"/Adminsubject/courseframe/cid/"+PackageCode;
                        },2000)
                    }
                }
            });
    ...
    $("#editAllNews").click(function(){
       ...
       $('#allEroar').html("");
		 $.ajax({
            url:site_url+"/Adminsubject/modsection",
            type:'post',
            data:{
                'cid':PackageCode,
