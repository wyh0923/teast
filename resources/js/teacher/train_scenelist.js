...
$(function(){
    $(".fa-search").click(function(){
        ...
        window.location.href= site_url + 'Train/scenelist' + "/search/"+encodeURI(translate(search));
    });
...
$(function(){
    ...
    $('.okBtn').click(function(){
        var code = $(this).attr('code');
        $.ajax({
            url:site_url+"Train/del_scene_tpl",
            type:'post',
            data:{'code':code},
            dataType:'json',
            success:function(message){
    ...
    $('.forBlue').click(function(){
        ...
        //检测用户是否有下发
        $.ajax({
            url:site_url+"Train/check_issue",
            type:'post',
    ...
    $('#scenestart').click(function(){
        ...
        var code = $(this).attr('code');
        $.ajax({
            url:site_url+"Train/start_scene",
            type:'post',
...
function enterSceneJudge(sceneInsUUID){
    $.ajax({
        url : site_url+"Train/check_scene_in_node",
        type : 'POST',
...
function scene(insuuid){
    ...
    $.ajax({
        url:site_url+"Train/enter_scene",
        type:"POST",
        ...
        success:function(message){
            if(message.code=='0000'){
                ...
                var url = site_url+'Train/vm_vnc?uuid='+insuuid+'&SectionInsNametitle='+scenename+'&token='+message.data.token+'&port='+message.data.port+'&ip='+message.data.ip+'&loguser='+ message.data.loguser+'&vmuuid='+ message.data.vmuuid + '&logpwd=' + message.data.logpwd + '&Sii=' + '' + '&host_id=' + message.data.host_id+ '&scene_end_time=' + message.data.SceneInstance.end_time;
...
function checkSceneStatus(){
    ...
    $.ajax({
        url:site_url+"Train/check_scene_status",
        type:'POST',
        async : false,
