...
$(function () {
    $(".fa-search").click(function () {
        ...
        window.location.href = site_url + 'Train/ctflist' + str + "/search/" + encodeURI(translate(search));
    });
...
$(function () {
    ...
    $('.forYellow').click(function () {
        var code = $(this).attr('code');
        $.ajax({
            url: site_url + "Train/get_ctf_info",
            type: 'post',
            data: {'code': code},
            dataType: 'json',
    ...
    var upedit = $('#uploadTool').Huploadify({
        ...
        buttonText: '上传资源',
        uploader: site_url + '/Train/upload_ctf',//服务器端脚本文件路径
    ...
    var upadd = $('#adduploadBox').Huploadify({//adduploadIcon
        formData: {key: timestamp, key2: 'ctf'},
        auto: true,//当选择文件后就直接上传了
        fileTypeExts: '*.jpg;*.png;*.gif;*.jpeg;*.zip;*.gzip;*.rar;*.doc;*.docx;*.xls;*.xlsx',//上传文件类型
        multi: false, //上传多个文件
            ...
            success: function (message) {
                ...
                if (message.code == '0000') {
                    ...
                    setTimeout(function () {
                        ...
                        location.href = site_url + 'Train/ctflist';
                    }, 2000);
                }
            }
        })
...
function searchForTime() {
    if ($("#stime").val() != "" && $("#etime").val() != "") {
        if ($("#stime").val() >= $("#etime").val()) {
            ...
            window.location.href = site_url + 'Train/ctflist' + str + '/time/' + $("#stime").val() + "_" + $("#etime").val()
        }
    }
...
function clearTime() {
    if ($("#stime").val() == "" && $("#etime").val() == "") {
        var str = '';
        if (search != '')str += '/search/' + encodeURI(translate(search));
        window.location.href = site_url + 'Train/ctflist' + str;
    }
}